...
class PatchedCausalStdScaler(nn.Module):
    ...
    def forward(
        ...
        return torch.where(mask, (data - loc) / scale, 0), loc, scale # 数据归一化. mask里面有数据的位置才归一化.否则写0.
    ...
    def _compute_loc_scale(self, data: torch.Tensor, mask: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        denominator = mask.cumsum(dim=-1).clamp_min(1) # 为了防止除0所以最小是1.
        causal_loc = cum_data / denominator  # 因果滑动均值

        # Welford-style causal variance 
        prev_loc = torch.cat([torch.zeros_like(causal_loc[..., :1]), causal_loc[..., :-1]], dim=-1) # 第一个元素是0, 其他元素是causal_loc的元素填补. 所以这一行等于shift(1)操作.
        delta = data - prev_loc  # 当前数据- 上一步的均值.
        increment = delta * (data - causal_loc) * mask  # 方差的增量.========这行代码就是welford公式. 详细推导可以看 https://www.bilibili.com/video/BV1BdviBWE1H/?vd_source=d68ed178f151e80fea1e02efd205802c  等于视频里面推导的 Mn+1-Mn.
        m_2 = torch.cumsum(increment, dim=-1)
        causal_var = m_2 / (denominator - self.correction).clamp(min=1) # 用于无偏估计.计算方差.
        causal_scale = torch.sqrt(causal_var).clamp(min=self.minimum_scale) # 标准差.
        ...
        return loc, scale # 最后得到的是均值, 和标准差.
...
class Toto2Model(nn.Module, PyTorchModelHubMixin):
        ...
        full_mask = torch.cat(
            [
                ...
                torch.ones(
                    ...
                    device=device,
                    dtype=torch.bool,
                ), # 这段代码含义是保证最后一个path在输入上是mask为1的. 也就是保证预测时候看到的最后一个path被mask掉.
                torch.zeros(
    ...
    def forecast(self, inputs, horizon, **kwargs):
        ...
        patch_size = self.config.patch_size
        num_patches = math.ceil(horizon / patch_size) # 预测的patch数量,这里面ceil表示要多预测保证patch数量能覆盖horizon. 需要多少再从里面挑选即可.
        ...
        if decode_block_size > 0:
            ...
            block_size_patches = num_patches # 需要预测的patch数量.
        ...
        initial_patches = math.ceil(initial_len / patch_size) # 输入的path数量.

        max_gids_len = max(initial_patches + num_patches, 2 * block_size_patches)
        base_gids = repeat(
            series_ids, "... n_var -> ... n_var seq", seq=max_gids_len,
        ).clone() # 把 series_ids 这个张量，在最后一个维度上重复 max_gids_len 次，生成一个新张量，然后复制一份（防止原地修改）。   base_gids.shape=1,1,19
        ctx_patch_obs = reduce(
            full_mask[..., :initial_len], "... (seq patch) -> ... seq", "sum", patch=patch_size,
        )  # 做掩码处理.  ctx_patch_obs.shape=1,1,16    因为full_mask的shape是512最后, 然后这里面把最后一个维度拆成seq patch, patch大小是32, 所以seq大小是16. 所以有16个patch. 每一个patch里面有32个数, 把这32个数进行sum就得到了这个ctx_patch_obs的可观测度. obs表示可观察的patch数量.
        base_gids[..., :initial_patches][ctx_patch_obs == 0] = -1 # 0观测度的部分设置为-1
        ...
        n_quantiles = len(self.output_head.knots) # 预测百分数的数量.
